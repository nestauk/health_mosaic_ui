<script>
  ...
  import { getLabel, noneLabel } from '../../../util/domain';
  import { countryLabel } from '../../../util/geo';
  import {
    getNodeDegree,
    getVolume,
    makeEmptyNetwork,
  ...
  const heroKey = 'countries_ids'; // FIXME pass the key as prop to make it generic
  const ignoredKey = 'ignoredMentionedCountries'; // FIXME pass the key as prop to make it generic
  const nodeKeyToLabel = _.pipe([
    countryLabel,
    _.when(_.isNil, _.always('none')),
  ]);
  const accessor = _.pipe([
    _.getKey(heroKey),
    _.condition(_.isNil, _.always([noneLabel]), _.identity),
  ]);
  const makeLinkIdToLabel = _.pipe([
    _.indexBy(getId),
    _.mapValuesWith(getLabel)
  ]);
  const makeNetwork = makeUndirectedNetworkWith(accessor, nodeKeyToLabel);
  ...
  $: subLinksIdToLabel = makeLinkIdToLabel(linksArray);
...
<div
  class="VolumeMentionedCountries"
  class:dirty="{isDirty}"
>
